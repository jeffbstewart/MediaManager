...
/**
 * Server-setup follow-up: username + password → authenticated session.
 *
 * Terms-of-use / privacy-policy acceptance is a **post-login** flow handled
 * by [LegalAgreementScreen]. The server's gRPC AuthInterceptor blocks any
 * non-compliant call with `PERMISSION_DENIED legal_agreement_required` until
 * the user agrees via AuthService.AgreeToTerms.
 */
@Composable
fun LoginScreen(
                ...
                Button(onClick = {
                    ...
                    scope.launch {
                        ...
                        try {
                            ...
                            TvLog.info("auth", "login success")
                            onLoginSuccess()
                        } catch (e: Exception) {
                            val bad = e.message?.contains("UNAUTHENTICATED") == true
                            TvLog.warn(
                                "auth",
                                if (bad) "login rejected (bad credentials)" else "login failed",
                                if (bad) null else e
                            )
                            error = if (bad) "Invalid username or password"
