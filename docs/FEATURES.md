...
| iOS CarPlay | Audio-only CarPlay surface with Albums / Playlists / Smart Playlists tabs, list-row thumbnails, Shuffle Library row, voice search via Siri INPlayMediaIntent (tracks, albums, artists, playlists), Now Playing hero, MPRemoteCommandCenter wiring for AirPods + lock screen + Control Center, auto-reconnect on stale HTTP/2 streams |
