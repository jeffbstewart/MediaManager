...
public actor ListeningProgressQueue {
    public static let shared = ListeningProgressQueue()
    ...
    public func enqueue(trackId: Int64, position: Double, duration: Double?) {
        ensureLoaded()
        queue[trackId] = Entry(
            trackId: trackId,
            position: position,
    ...
    /// each successful RPC.
    public func remove(trackId: Int64) {
        ensureLoaded()
