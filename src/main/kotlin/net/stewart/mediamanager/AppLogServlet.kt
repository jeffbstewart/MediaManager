...
/**
 * Serves the in-memory application log at /admin/logs.
 *
 ...
 *   level=info     — all (default)
 *   logger=<substr> — filter by logger name substring
 */
class AppLogServlet : HttpServlet() {

    private val gson = GsonBuilder().setPrettyPrinting().create()

    override fun doGet(req: HttpServletRequest, resp: HttpServletResponse) {
        val levelFilter = req.getParameter("level")?.uppercase()
        val loggerFilter = req.getParameter("logger")

        var entries = when (levelFilter) {
        ...
        if (loggerFilter != null) {
            entries = entries.filter { it.loggerName.contains(loggerFilter, ignoreCase = true) }
        }
    ...
    private fun respondJson(resp: HttpServletResponse, entries: List<AppLogBuffer.LogEntry>) {
    ...
    private fun respondHtml(resp: HttpServletResponse, entries: List<AppLogBuffer.LogEntry>) {
