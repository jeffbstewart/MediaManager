...
@Route(value = "content/tv", layout = MainLayout::class)
@PageTitle("TV Shows")
class TvShowsView : KComposite() {

    private var playableOnly: Boolean = true
    private var selectedRatings: Set<String> = emptySet()
    private var sortMode: TvSortMode = TvSortMode.NAME
    ...
    private fun buildChips() {
        ...
        val allRatings = listOf("TV-Y", "TV-Y7", "TV-G", "TV-PG", "TV-14", "TV-MA")
        ...
        // Sort chips
        chipRow.add(createSeparator())
        for (mode in TvSortMode.entries) {
    ...
    private fun refreshGrid() {
        ...
        titles = when (sortMode) {
            TvSortMode.NAME -> titles.sortedBy { (it.sort_name ?: it.name).lowercase() }
            TvSortMode.YEAR -> titles.sortedByDescending { it.release_year ?: 0 }
            TvSortMode.RECENTLY_ADDED -> titles.sortedByDescending { it.created_at }
            TvSortMode.POPULARITY -> titles.sortedByDescending { it.popularity ?: 0.0 }
        }
        ...
        statusLabel.text = "${titles.size} shows"
    }
...
private enum class TvSortMode(val label: String) {
    NAME("Name"),
    YEAR("Year"),
